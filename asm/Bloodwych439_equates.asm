...
ActorForm_MonsterRendererFirst:							equ	$64			; Converts the monster type code into the renderer dispatch index; codes below the first monster type continue to character drawing.

ActorRecord_ActionState:								equ	$05			; Clears the live monster action/status byte.
ActorRecord_BehaviourType:								equ	$0A			; Reads the live monster type for repacking.
ActorRecord_FacingMask:									equ	$03			; Mask selecting the low two facing bits from packed actor rotation/space state.
ActorRecord_Floor:										equ	$04			; Copies the floor while compacting team records.
ActorRecord_Form:										equ	$0B			; Writes the live form into the packed record.
ActorRecord_HitPoints:									equ	$08			; Stores calculated starting hit points.
ActorRecord_MiniSpaceMask:								equ	$F0			; Mask selecting the high-nibble mini-space value from packed actor rotation/space state.
ActorRecord_MiniSpaceShift:								equ	$04			; Shifts the live monster rotation state down from its high nibble before viewer-relative facing is calculated.
ActorRecord_NoPosition:									equ	$FF			; Marks the source live record as removed.
ActorRecord_RotationAndSpace:							equ	$02			; Reads the rotation or occupied-space field while rebuilding teams.
ActorRecord_Size:										equ	$10			; Advances the live monster offset to the next record.
ActorRecord_SizeShift:									equ	$04			; Converts a live monster byte offset into a record index.
...
ChampionIndex_Eleanor:									equ	$02			; Champion index for Eleanor.
ChampionIndex_Elfric:									equ	$07			; Champion index for Elfric.
ChampionIndex_Hengist:									equ	$0E			; Champion index for Hengist.
ChampionIndex_Megrim:									equ	$09			; Champion index for Megrim.
ChampionIndex_MrFlay:									equ	$0B			; Champion index for Mr Flay.
ChampionIndex_Murlock:									equ	$01			; Champion index for Murlock.
ChampionIndex_Rosanne:									equ	$03			; Champion index for Rosanne.
ChampionIndex_Sethra:									equ	$0A			; Champion index for Sethra.
ChampionIndex_SirEdward:								equ	$08			; Champion index for Sir Edward.
ChampionIndex_ThaiChang:								equ	$0F			; Champion index for Thai Chang.
ChampionIndex_Ulrich:									equ	$0C			; Champion index for Ulrich.
ChampionIndex_Zastaph:									equ	$0D			; Champion index for Zastaph.
...
ChampionStat_ActionCycleTimer:							equ	$19			; Champion movement-speed field.
ChampionStat_ActionState:								equ	$1B			; Clears the champion attack cooldown.
...
ChampionStatsScroll_FieldCount:							equ	$07			; Number of ordinary two-digit fields inserted by the champion stats-scroll lookup.
...
CharacterActionState_PhysicalAttackStart:				equ	$07			; Initial cooldown written whenever a champion performs a physical attack.
CharacterActionState_RenderIndexMask:					equ	$1F			; Mask retaining the five monster render-state bits used by the animation lookup.
CharacterActionState_TerrorBit:							equ	$05			; Bit marking Terror in a character action-state byte.

CharacterActionTimer_CommsCountdown:					equ	$0A			; Action-cycle countdown forced when communication starts.
CharacterActionTimer_CountdownMask:						equ	$0F			; Mask selecting the current low-nibble action-cycle countdown.
CharacterActionTimer_MaxCountdown:						equ	$0F			; Maximum low-nibble action-cycle countdown.
CharacterActionTimer_ReloadMask:						equ	$F0			; Mask selecting the high-nibble action-cycle reload interval.
CharacterActionTimer_ReloadShift:						equ	$04			; Shift moving the action-cycle reload nibble into countdown position.
...
PlayerData_DialogueColourState:							equ	$52			; Offset of the dialogue colour and fade state byte.
PlayerData_DialogueColourState_AlternateRampBit:		equ	$06			; Bit selecting the alternate dialogue colour ramp.
PlayerData_DialogueColourState_AutoFadeBit:				equ	$00			; Bit enabling automatic dialogue-colour fading.
PlayerData_DialogueColourState_DeferFadeUntilFlushBit:	equ	$07			; Bit deferring dialogue fading until the pending UI copy is flushed.
PlayerData_DialogueFadeRampStep:						equ	$4B			; Offset of the dialogue colour-ramp step byte.
PlayerData_DialogueFadeStepCountdown:					equ	$4A			; Offset of the dialogue fade-step countdown byte.
PlayerData_Direction:									equ	$20			; Stores the champion facing direction in the active player record.
PlayerData_EngagedActorIndex:							equ	$35			; Offset of the engaged or controlled live-actor index.
PlayerData_FairyOfferDelayCountdown:					equ	$3F			; Offset of the sleeping/fairy interaction delay countdown.
PlayerData_Floor:										equ	$58			; Stores the champion floor in the active player record.
PlayerData_FormationSlotsOffset:						equ	$26			; Base offset of the four party formation/display-order champion slots.
...
PlayerData_InterfaceRepeatSnapshot:						equ	$24			; Offset of the previous interface repeat/countdown timer word.
PlayerData_InterfaceScreenBufferOffset:					equ	$0A			; Offset of the player-specific interface framebuffer byte offset.
PlayerData_MousePosition:								equ	$02			; Offset of the packed mouse X/Y position longword.
PlayerData_MouseYClampBounds:							equ	$3A			; Offset of the packed mouse-Y maximum and minimum clamp bytes.
PlayerData_MouseYMaximum:								equ	$3A			; Offset of the mouse-Y maximum clamp byte.
PlayerData_MouseYMinimum:								equ	$3B			; Offset of the mouse-Y minimum clamp byte.
PlayerData_MouseYPosition:								equ	$04			; Offset of the mouse Y word within the packed mouse position.
PlayerData_PartyCommandDescriptorPointer:				equ	$46			; Offset of the party-command descriptor pointer.
PlayerData_PartyCommandSelection:						equ	$40			; Offset of the current party-command selection word; $FFFF means no selection.
PlayerData_PartyCommandSelectionSubIndex:				equ	$41			; Offset of the party-command selection sub-index byte.
PlayerData_PartyCommandState:							equ	$42			; Offset of the party-command interface state word.
PlayerData_PartyShieldHighlightCountdowns:				equ	$5A			; Base offset of four party shield or wound-highlight countdowns.
PlayerData_PendingAction:								equ	$56			; Offset read when transferring a pending action into the active command.
...
PlayerData_SpellBookPageSpread:							equ	$2A			; Offset of the spellbook page-spread index word.
PlayerData_SpellCastBarState:							equ	$57			; Offset of the spell-casting bar state byte.
PlayerData_StateFlags:									equ	$00			; Offset of the PlayerData state and action flags byte.
PlayerData_StateFlags_DisplayCentreAttackBit:			equ	$03			; Bit enabling the centre attack display/action state.
PlayerData_StateFlags_PlayerIdentityBit:				equ	$00			; Bit distinguishing the two PlayerData records in shared interface code.
PlayerData_StateFlags_PrimaryAttackBit:					equ	$01			; State bit set by the primary attack handler.
PlayerData_StateFlags_SleepingBit:						equ	$02			; Bit marking that the player's party is sleeping.
PlayerData_UIDirtyRegionFlags:							equ	$54			; Offset of the UI dirty-region bitfield consumed by the screen-band copy code.
PlayerData_UIDirtyRegionFlags_CentreLower8RowsBit:		equ	$04			; Dirty-region bit selecting the centre lower eight-row strip.
PlayerData_UIDirtyRegionFlags_CentreMainBit:			equ	$06			; Dirty-region bit selecting the centre main interface band.
PlayerData_UIDirtyRegionFlags_LeftExtend8RowsBit:		equ	$01			; Dirty-region bit extending the left main band by eight rows.
PlayerData_UIDirtyRegionFlags_LeftMainBit:				equ	$00			; Dirty-region bit selecting the left main interface band.
PlayerData_UIDirtyRegionFlags_LeftTop7RowsBit:			equ	$05			; Dirty-region bit selecting the left top seven-row strip.
PlayerData_UIDirtyRegionFlags_RightExtend8RowsBit:		equ	$03			; Dirty-region bit extending the right main band by eight rows.
PlayerData_UIDirtyRegionFlags_RightMainBit:				equ	$02			; Dirty-region bit selecting the right main interface band.
PlayerData_UIDirtyRegionFlags_RightTop7RowsBit:			equ	$07			; Dirty-region bit selecting the right top seven-row strip.
...
PlayerData_ViewportFillInk:								equ	$3D			; Offset of the viewport fill or transition ink byte.
PlayerData_WaitingChampionRecallFlag:					equ	$50			; Offset of the flag controlling waiting-champion recall processing.
PlayerData_XPosition:									equ	$1C			; Stores the champion start X position in the active player record.
...
SpellCastBarState_Enabled:								equ	$00			; State value enabling the spell-casting bar.
